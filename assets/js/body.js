...
async function loadPosts(page = 1) {
  ...
  try {
    ...
    snapshot.forEach(doc => {
      const post = doc.data(); post.id = doc.id; allPosts.push(post);
    });

    let filteredPosts = allPosts;
    if (window.currentCategoryId) filteredPosts = filteredPosts.filter(p => p.category === window.currentCategoryId);
    if (window.currentSubcategoryId) filteredPosts = filteredPosts.filter(p => p.subcategory === window.currentSubcategoryId);
    if (currentAuthorId) filteredPosts = filteredPosts.filter(p => p.authorId === currentAuthorId);
    ...
    const bodyFont = settings.bodyFont || 'Amiri';
...
async function createPostCard(post, titleFont = 'Cinzel', bodyFont = 'Amiri') {
      ...
      switch (item.type) {
        ...
        case 'images':
          if (item.images) item.images.forEach(img => {
            contentHTML += `<img src="${img.dataUrl || img}" class="post-image" loading="lazy" onerror="this.style.display='none'">`;
          });
          break;
        case 'code': contentHTML += `<pre class="code-block"><code class="language-${item.language}">${escapeHTML(item.value)}</code></pre>`; break;
        case 'link': contentHTML += `<a href="${item.url}" target="_blank" rel="noopener" class="post-link">🔗 ${item.text || item.url}</a>`; break;
  ...
  card.innerHTML = `
    ...
    <div class="post-body" style="font-family: '${bodyFont}', serif;">
...
async function incrementViewCount(postId) {
  ...
  } catch (e) { console.error('خطأ في تحديث المشاهدات:', e); }
}
